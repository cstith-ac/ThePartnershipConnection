...
namespace WebAPI_CMS.Controllers
{
    ...
    [ApiController]
    public class SiteSystemDetailsController : ControllerBase
    {
        public HttpWebRequest CreateSOAPRequest()
        {
            ...
            //SOAPAction
            Req.Headers.Add(@"SOAPAction:http://tempuri.org/GetSiteDetail");
            //Content_type
        ...
        [HttpPost("{id1}")]
        public IActionResult GetSiteSystemDetails(string id1)
        {
            ...
            using (WebResponse ServiceRes = request.GetResponse())
            {
                using (StreamReader rd = new StreamReader(ServiceRes.GetResponseStream()))
                {
                    ...
                    //xDoc.Save("assets/SiteSystemDetailsTest.xml");

                    XDocument xml = XDocument.Parse(ServiceResult);
                    var soapResponse = xml.Descendants().Where(x => x.Name.LocalName == "SiteDetail").Select(x => new SiteSystemDetails()
                    {
                        ...
                        active_date = (string)x.Element(x.Name.Namespace + "active_date")
                    }).ToList();
