...
export class IncentiveEntryService {
  ...
  updateRecurring(ItemID: number, Description: string, BillCycle: string, RMR: number, PassThrough: number, BillingStartDate: Date, Multiple: number, Add2Item: number, Total: number) {
    console.log('updateRecurring');
    this.sharedIncentiveRecurringInfo.push({ ItemID, Description, BillCycle, RMR, PassThrough, BillingStartDate, Multiple, Add2Item, Total })
  }
